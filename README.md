...
HTTPS 인증서 적용, 방화벽 설정까지 운영 환경 수준으로 확인했습니다.
...
### 보안
- ufw + Oracle Security List 이중 방화벽 설정 (22, 80, 443만 허용)
- SSH 키 인증 전용 설정 (패스워드 로그인 비활성화)
- Let's Encrypt + certbot HTTPS 인증서 발급 및 자동 갱신 (certbot renew 크론 등록)
...
- Bash 기반 `setup_ssl.sh` (certbot 설치 → 인증서 발급 → 자동 갱신 크론 등록)
...
[ Duck DNS ] → Oracle Cloud VM 공인 IP
    |
    v
[ ufw + Oracle Security List ] (22, 80, 443만 허용)
    |
    v
[ Nginx (443 SSL) ]
    |
...
production 배포에서는 프론트엔드를 Nginx로 정적 서빙하고,
`/api` 요청은 backend 컨테이너로 프록시되도록 구성했습니다.
80포트는 443으로 리다이렉트되어 모든 요청이 HTTPS로 처리됩니다.

배포용 설정은 아래 파일들을 기준으로 분리했습니다.

- `frontend/Dockerfile.prod`
- `frontend/nginx.conf` (envsubst 기반 도메인 템플릿)
- `docker-compose.yml`
- `.env.prod`
...
- Oracle Security List에서 22, 80, 443 Ingress 허용
- ufw 설정 완료
- SSH 키 인증 설정 완료
...
### 1. 서버 방화벽 설정
```bash
# iptables (Oracle Cloud Ubuntu 기본 차단 규칙 해제)
...
# DOMAIN, CORS_ALLOW_ORIGINS 등 실제 값으로 수정
```

`.env.prod` 주요 항목:
```env
DOMAIN=your-name.duckdns.org
CORS_ALLOW_ORIGINS=https://your-name.duckdns.org
```
...
`setup_ssl.sh` 실행 시 아래 순서로 처리됩니다.
1. certbot 설치
2. 컨테이너 중지 (standalone 모드를 위한 80포트 확보)
3. Let's Encrypt 인증서 발급
4. 자동 갱신 크론 등록 (매일 새벽 3시)
...
### 6. 주요 확인 포인트
- `https://your-name.duckdns.org` 외부 접속 확인
- `/api/health/live` 응답 확인
- `/api/health/ready` 응답 확인
- `docker compose logs backend --tail 100` 로그 확인
...
| `setup_ssl.sh` | certbot 설치, 인증서 발급, 자동 갱신 크론 등록 |
...
- `remote_deploy.sh`를 통한 배포 흐름 재현
...
- Oracle Cloud VM 공인 IP 기반 외부 접속 환경 구성
- 방화벽 설정 (ufw + Oracle Security List)
- SSH 키 인증 전용 설정
- Let's Encrypt HTTPS 인증서 발급 및 자동 갱신
- RUNBOOK 초안 작성
